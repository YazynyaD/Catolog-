...
def ct1(brow,ur,otr1,otr2,urli,notwww,namecl,unamecl,city,cityr,ciadress,street,home,adress,numclinic,numtrue,namepers,fio,f,i,o,timework,url,mail,keysl,desc,tupecl,passw,login,numcodcity,numclshot,numn,numpl,shcir,regi,ob,bb,ind):
    ...
    try:
        ...
        pass
    try:
        brow.find_element(By.XPATH, "/html/body/div[3]/div[3]/div/div[2]/div/form/div[6]/img").click()
        
    except Exception:
        print("Ошибка: done /baza-r.ru")
        pass
    try:
        curl=brow.find_element(By.XPATH, "/html/body/div[3]/div[3]/div/div[2]/div/p[3]/span").text
        print(curl)
        df = pd.DataFrame({'Карточка': [curl, 188500000]})
        df.to_excel('./st/autost/ex/ur.xlsx') 
        
    except Exception:
        print("Ошибка: get /baza-r.ru")
        curl="non4"
        df = pd.DataFrame({'Карточка': [curl, 188500000]})
        df.to_excel('./st/autost/ex/ur.xlsx') 
        pass
